...
export default function Attendance() {
  const { records, loading, fetchAttendanceRecords, fetchAllUsersAttendance, markAttendance } = useAttendanceStore();
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      ...
      const usersData = querySnapshot.docs.reduce((acc, doc) => {
        ...
        if (userData.verified) { // Only include verified users
          return {
            ...acc,
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Total Days Present: {getTotalAttendance(selectedUser || user?.uid || '')}
          </p>
        </div>
        {/* {!isAdmin && ( */}
          <button
          ...
          >
            ...
            {Object.values(users).map((user) => (
              <option key={user.id} value={user.id}>
                {user.fullName} - {getTotalAttendance(user.id)} days present
              </option>
            ))}
          </select>
        </div>
